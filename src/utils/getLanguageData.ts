...
const languageData = {
	...
	"en-US": {
		...
		loveSongRemoveError: "An error occurred while unliking music",
 ...
 */
const getLanguageData = (type: keyof (typeof languageData)["en-US"]) => {
	try {
		const { language } = store.getState().setting;
